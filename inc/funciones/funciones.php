...
function obtenerProyectosCompletados(){
    global $conn;
    try{
        return $conn->query("SELECT proyecto_historico.proyecto,
                            ...
                            FROM ((proyecto_historico 
                                    INNER JOIN alumno ON proyecto_historico.id_alumno=alumno.id)  INNER JOIN profesor ON profesor.id = proyecto_historico.id_asesor1)");
    } catch(Exception $e){
        echo "Error!!!".$e->getMessage()."<br>";
        return false;
    } 
...
function obtenerProyectosEnProceso(){
    global $conn;
    try{
        return $conn->query("SELECT proyecto_vigente.proyecto,profesor.nombre as nom_asesor, profesor.apellido as ap_asesor, alumno.nombre,alumno.apellido,proyecto_vigente.fechaInicio,proyecto_vigente.fechaFin,proyecto_vigente.descripcion FROM ((proyecto_vigente INNER JOIN alumno ON proyecto_vigente.id_alumno=alumno.id) INNER JOIN profesor ON profesor.id = proyecto_vigente.id_asesor1) WHERE proyecto_vigente.id > 1");
    } catch(Exception $e){
        echo "Error!!!".$e->getMessage()."<br>";
        return false;
    } 
